...
describe('plugin', () => {
  it('should create and read TODO items', async () => {
    const { server } = await startTestBackend({
      features: [facetsCloudPlugin],
    });

    await request(server).get('/api/facets-cloud/todos').expect(200, {
      items: [],
    });

    const createRes = await request(server)
      .post('/api/facets-cloud/todos')
      .send({ title: 'My Todo' });
    ...
    await request(server)
      .get('/api/facets-cloud/todos')
      .expect(200, {
        items: [createdTodoItem],
      });

    await request(server)
      .get(`/api/facets-cloud/todos/${createdTodoItem.id}`)
